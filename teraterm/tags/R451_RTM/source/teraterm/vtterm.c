...
// (2005.2.20 yutaka)
  if (ts.LogTypePlainText) {
	  if (__isascii(b) && !isprint(b)) {
		  // ASCII•¶š‚ÅA”ñ•\¦‚È•¶š‚ÍƒƒOÌæ‚µ‚È‚¢B
	  } else {
		if (cv.HLogBuf!=0) Log1Byte(b);
	  }
  } else {
...
void CSSetAttr()
{
	...
	for (i=1 ; i<=NParam ; i++)
	{
		...
		case 27:
			...
			break;
		default:
			// ESC[39m‚ª‘OŒiF‚ÌƒŠƒZƒbƒg‚ÅESC[49m‚ª”wŒiF‚Å‚ ‚étermcap xterm‚Ì
			// ƒGƒXƒP[ƒvƒV[ƒPƒ“ƒX‚ğ’Ç‰ÁiŠâ–{ƒpƒbƒ`jB 
			// ‚±‚ê‚É‚æ‚èscreenã‚Åw3m‚ğg—p‚µ‚½ê‡AF‚ª–ß‚ç‚È‚¢Œ»Û‚ª‰ü‘P‚³‚ê‚éB
			// (2005/4/7 yutaka)
...
void XSequence(BYTE b)
{
  if (NParam==1)
  {
    ...
    if (b<US) {
      ...
      {
	ts.Title[NParam-2] = 0;
	// (2006.6.15 maya) ƒ^ƒCƒgƒ‹‚É“n‚·•¶š—ñ‚ğSJIS‚É•ÏŠ·
...
static void UnicodeToCP932(unsigned int code, int byte)
{
	...
	if (ret != -1) {
		...
		if (_stricmp(ts.Locale, DEFAULT_LOCALE) == 0) {
			// U+301C‚È‚Ç‚Í•ÏŠ·‚Å‚«‚È‚¢BUnicode -> Shift_JIS‚Ö•ÏŠ·‚µ‚Ä‚İ‚éB
...
//  (actually allways returns TRUE)
{
	...
	if ((b & 0x80) != 0x80) { // ASCII(0x00-0x7f)
		// 1ƒoƒCƒg–Ú‚¨‚æ‚Ñ2ƒoƒCƒg–Ú‚ªASCII‚Ìê‡‚ÍA‚·‚×‚ÄASCIIo—Í‚Æ‚·‚éB
		if (count == 0 || count == 1) {
	...
	if ((buf[0] & 0xe0) == 0xc0 && 
		...
		// Ÿ‚ÌƒoƒCƒg‚ªASCII‚È‚ç‚»‚Ì‚Ü‚Ü•\¦ (2006.6.30 yutaka)
		if ((b & 0x80) != 0x80) { // ASCII(0x00-0x7f)
			ParseASCII(b);
			count = 0;  // reset counter

		} else {
			// ASCII‚Å‚È‚¢‚Ì‚È‚çAƒ}ƒ‹ƒ`ƒoƒCƒg•¶š‚Ìn‚Ü‚è‚ÆŒ©‚È‚·B
			buf[0] = b;
			count = 1;
		}
	...
	if ((buf[0] & 0xe0) == 0xe0 && 
		(buf[1] & 0xc0) == 0x80 && 
		(buf[2] & 0xc0) == 0x80) { // 3ƒoƒCƒgƒR[ƒh‚Ìê‡
		...
		if (hfsplus_mode == 1) {
			if (maybe_hfsplus == 0) {
				...
				if (cset != 0) { // success
					...
					// 2‚Â‚ß‚Ì•¶š‚ª”¼‘÷“_‚Ì1•¶š–Ú‚É‘Š“–‚·‚éê‡‚ÍAÄ“xŒŸõ‚ğ‘±‚¯‚éB(2005.10.15 yutaka)
					if ((first_code_index = GetIndexOfHFSPlusFirstCode(
							code, mapHFSPlusUnicode, sizeof(mapHFSPlusUnicode)/sizeof(mapHFSPlusUnicode[0])
							)) != -1) {

						// 1‚Â‚ß‚Ì•¶š‚Í‚»‚Ì‚Ü‚Üo—Í‚·‚é
						UnicodeToCP932(first_code, 3);
 ...
 * ƒtƒ@ƒCƒ‹‚Ì‰üsƒR[ƒh‚ğ@LF ‚É‚µ‚½B
 *
 * Revision 1.10  2006/06/15 16:27:59  yutakakn
 * ƒ^ƒCƒgƒ‹‚É“ú–{Œê‚ğ“ü—Í‚·‚é‚ÆA•¶š‰»‚¯‚·‚éƒoƒO‚ğC³‚µ‚½Bƒpƒbƒ`ì¬‚ÉŠ´Ó‚µ‚Ü‚·„‰i“c
 *
 ...
 * UTF-8(Mac OS X)‚É‚¨‚¢‚Ä”¼‘÷“_•¶š‚ª•\¦‚³‚ê‚È‚¢‚±‚Æ‚ª‚ ‚éƒoƒO‚ğC³‚µ‚½B
 *
 ...
 * <ESC>[J‚É‚æ‚é‰æ–ÊƒNƒŠƒA‚ÉƒJƒŒƒ“ƒgƒoƒbƒtƒ@‚ğƒXƒNƒ[ƒ‹ƒAƒEƒg‚³‚¹‚é‚æ‚¤‚É‚µ‚½B
 *
 * Revision 1.5  2005/04/07 12:51:34  yutakakn
 * ƒGƒXƒP[ƒvƒV[ƒPƒ“ƒXiESC[39m, ESC[49mj‚ğƒTƒ|[ƒg‚µ‚½B‚±‚ê‚É‚æ‚èscreenã‚Åw3m‚ğg—p‚µ‚½ê‡AF‚ª–ß‚ç‚È‚¢Œ»Û‚ª‰ü‘P‚³‚ê‚éBŠâ–{‚ÉŠ´Ó‚µ‚Ü‚·B
 *
 * Revision 1.4  2005/02/28 13:54:47  yutakakn
 * ‘SŠp•¶š‚ªs––‚ğŒ×‚é‚ÆAã•û‚ÖƒXƒNƒ[ƒ‹‚µ‚Ä‚µ‚Ü‚¤–â‘è‚Ö‚Ì‘ÎˆiŠâ–{ƒpƒbƒ`jB
 * http://www.freeml.com/message/teraterm@freeml.com/0000142
 *
 * Revision 1.3  2005/02/20 14:51:29  yutakakn
 * ƒƒOƒtƒ@ƒCƒ‹‚Ìí•Ê‚É"plain text"‚ğ’Ç‰ÁB‚±‚ÌƒIƒvƒVƒ‡ƒ“‚ª—LŒø‚Ìê‡‚ÍAƒƒOƒtƒ@ƒCƒ‹‚É
 * ASCII”ñ•\¦•¶š‚ÌÌæ‚ğ‚µ‚È‚¢B
 *
 * Œ»İA–³‹‚·‚éƒLƒƒƒ‰ƒNƒ^‚ÍˆÈ‰º‚Ì‚Æ‚¨‚èB
 * @EBS
 * @EASCII(0x00-0x1f)‚Ì‚¤‚¿”ñ•\¦‚È‚à‚Ì
 *
 * ‚½‚¾‚µA—áŠO‚Æ‚µ‚ÄˆÈ‰º‚Ì‚à‚Ì‚ÍƒƒOÌæ‘ÎÛB
 * @EHT
 * @ECR
 * @ELF
 *
 * Revision 1.2  2004/12/07 14:31:13  yutakakn
 * s‚ª˜AŒ‹‚µ‚Ä‚¢‚éê‡‚ÍAƒƒOƒtƒ@ƒCƒ‹‚É‰üsƒR[ƒh‚ğŠÜ‚ß‚È‚¢‚æ‚¤‚É‚µ‚½B
 * ‚½‚¾‚µAEnableContinuedLineCopy ‹@”\‚ª—LŒø‚Ìê‡‚ÉŒÀ‚éB
 * 
 */
