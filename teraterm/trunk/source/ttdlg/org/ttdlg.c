...
static char *MouseCursorList[] = {
	"ARROW", "CROSS", "IBEAM", "HAND", "SIZENS", NULL
};
...
// TermJ_Term[]とTermListJ[]の個数は同期を取ること(yutaka)
...
static PCHAR far KanjiList[] = {"SJIS","EUC","JIS", "UTF-8", NULL};
  ...
  switch (Message) {
	...
	DisableDlgItem(Dialog,IDC_WINHIDEMENU,IDC_WINHIDEMENU);
      ...
      if (ts->VTFlag>0)
      {
		  ...
		  // mouse cursor
		  {
		  char **p = MouseCursorList;
		  while (*p) {
              SendDlgItemMessage(Dialog, IDC_MOUSECURSOR, CB_ADDSTRING,
				0, *p);
              p++;
		  }
          SendDlgItemMessage(Dialog, IDC_MOUSECURSOR, CB_SETCURSEL,
			0, 0);
		  }
#endif
      }
