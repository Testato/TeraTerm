...
BOOL CALLBACK WinDlg(HWND Dialog, UINT Message, WPARAM wParam, LPARAM lParam)
{
  ...
  switch (Message) {
	...
	// ì¸óÕç≈ëÂåÖêîÇ 5 Ç©ÇÁ 8 Ç÷ägí£ (2004.11.28 yutaka)
...
// É{Å[ÉåÅ[ÉgÇÃè„å¿Çägí£ÇµÇΩ (2005.11.30 yutaka)
  ...
  switch (Message) {
	...
	case IDOK:
	  ...
	  if ( ts!=NULL )
	  {
	    ...
	    // É{Å[ÉåÅ[ÉgÇ™ïœçXÇ≥ÇÍÇÈÇ±Ç∆Ç™Ç†ÇÈÇÃÇ≈ÅAÉ^ÉCÉgÉãçƒï\é¶ÇÃ
	    // ÉÅÉbÉZÅ[ÉWÇîÚÇŒÇ∑ÇÊÇ§Ç…ÇµÇΩÅB (2007.7.21 maya)
...
BOOL CALLBACK HostDlg(HWND Dialog, UINT Message, WPARAM wParam, LPARAM lParam)
{
  ...
  switch (Message) {
    case WM_INITDIALOG:
      ...
      get_lang_msg("DLG_HOST_TCPIP_HOST", uimsg, sizeof(uimsg), uimsg2, UILanguageFile);
      SetDlgItemText(Dialog, IDC_HOSTNAMELABEL, uimsg);
      GetDlgItemText(Dialog, IDC_HOSTTCPPORTLABEL, uimsg2, sizeof(uimsg2));
      get_lang_msg("DLG_HOST_TCPIP_PORT", uimsg, sizeof(uimsg), uimsg2, UILanguageFile);
      SetDlgItemText(Dialog, IDC_HOSTTCPPORTLABEL, uimsg);
      GetDlgItemText(Dialog, IDC_HOSTTCPPROTOCOLLABEL, uimsg2, sizeof(uimsg2));
      get_lang_msg("DLG_HOST_TCPIP_PROTOCOL", uimsg, sizeof(uimsg), uimsg2, UILanguageFile);
      ...
      get_lang_msg("DLG_HOST_SERIAL_PORT", uimsg, sizeof(uimsg), uimsg2, UILanguageFile);
...
//
// static textÇ…èëÇ©ÇÍÇΩURLÇÉ_ÉuÉãÉNÉäÉbÉNÇ∑ÇÈÇ∆ÅAÉuÉâÉEÉUÇ™ãNìÆÇ∑ÇÈÇÊÇ§Ç…Ç∑ÇÈÅB
...
// static textÇ…äÑÇËìñÇƒÇÈÉvÉçÉVÅ[ÉWÉÉ
static LRESULT CALLBACK UrlWndProc( HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam )
{
	...
	case WM_TIMER:
		// URLÇÃè„Ç…É}ÉEÉXÉJÅ[É\ÉãÇ™Ç†ÇÈÇ»ÇÁÅAÉVÉXÉeÉÄÉJÅ[É\ÉãÇïœçXÇ∑ÇÈÅB
		if (author_url_class.mouseover || forum_url_class.mouseover) {
			...
			SetSystemCursor(CopyCursor(hc), 32512 /* OCR_NORMAL */);    // ñÓàÛ
			SetSystemCursor(CopyCursor(hc), 32513 /* OCR_IBEAM */);     // IÉrÅ[ÉÄ

		} else {
			//ReleaseCapture();
			// É}ÉEÉXÉJÅ[É\ÉãÇå≥Ç…ñﬂÇ∑ÅB
			SystemParametersInfo(SPI_SETCURSORS, 0, NULL, 0);

		}

		// ÉJÅ[É\ÉãÇ™ÉEÉBÉìÉhÉEäOÇ…Ç†ÇÈèÍçáÇ…Ç‡ WM_MOUSEMOVE ÇëóÇÈ
		GetCursorPos( &pt );
	...
	case WM_PAINT: 
		...
		// åªç›ÇÃÉNÉâÉCÉAÉìÉgãÈå`ÅAÉeÉLÉXÉgÅAÉtÉHÉìÉgÇéÊìæÇ∑ÇÈ
		GetClientRect( hWnd, &rc );
	...
	case WM_ERASEBKGND:
		...
		// îwåiï`âÊ
		if( parent->mouseover ){
			// ÉnÉCÉâÉCÉgéûîwåiï`âÊ
			SetBkColor( hdc, RGB( 0xff, 0xff, 0 ) );
			ExtTextOut( hdc, 0, 0, ETO_OPAQUE, &rc, NULL, 0, NULL );
		}else{
			// êeÇ…WM_CTLCOLORSTATICÇëóÇ¡ÇƒîwåiÉuÉâÉVÇéÊìæÇµÅAîwåiï`âÊÇ∑ÇÈ
			HBRUSH hbr;
			HBRUSH hbrOld;
...
// static textÇ…ÉvÉçÉVÅ[ÉWÉÉÇê›íËÇµÅAÉTÉuÉNÉâÉXâªÇ∑ÇÈÅB
static void do_subclass_window(HWND hWnd, url_subclass_t *parent)
{
	...
	// êeÇÃÉvÉçÉVÅ[ÉWÉÉÇÉTÉuÉNÉâÉXÇ©ÇÁéQè∆Ç≈Ç´ÇÈÇÊÇ§Ç…ÅAÉ|ÉCÉìÉ^Çìoò^ÇµÇƒÇ®Ç≠ÅB
	lptr = SetWindowLongPtr( hWnd, GWLP_USERDATA, (LONG_PTR)parent );
	// ÉTÉuÉNÉâÉXÇÃÉvÉçÉVÅ[ÉWÉÉÇìoò^Ç∑ÇÈÅB
	...
	lf.lfUnderline = TRUE;
	parent->font = hFont = CreateFontIndirect( &lf ); // ïsóvÇ…Ç»Ç¡ÇΩÇÁçÌèúÇ∑ÇÈÇ±Ç∆
	if (hFont != NULL)
...
#endif
{
	...
	case WM_INITDIALOG:
		...
		// TeraTermÇÃÉoÅ[ÉWÉáÉìÇê›íËÇ∑ÇÈ (2005.2.28 yutaka)
		// __argv[0]Ç≈ÇÕ WinExec() ÇµÇΩÉvÉçÉZÉXÇ©ÇÁéQè∆Ç≈Ç´Ç»Ç¢ÇÊÇ§Ç»ÇÃÇ≈çÌèúÅB(2005.3.14 yutaka)
		...
		// OnigurumaÇÃÉoÅ[ÉWÉáÉìÇê›íËÇ∑ÇÈ 
		// ÉoÅ[ÉWÉáÉìÇÃéÊìæÇÕ onig_version() ÇåƒÇ—èoÇ∑ÇÃÇ™ìKêÿÇæÇ™ÅAÇªÇÃÇΩÇﬂÇæÇØÇ…ÉâÉCÉuÉâÉäÇ
		// ÉäÉìÉNÇµÇΩÇ≠Ç»Ç©Ç¡ÇΩÇÃÇ≈ÅAà»â∫ÇÃÇÊÇ§Ç…ÇµÇΩÅBOnigurumaÇÃÉoÅ[ÉWÉáÉìÇ™è„Ç™Ç¡ÇΩèÍçáÅA
		// ÉrÉãÉhÉGÉâÅ[Ç∆Ç»ÇÈÇ©Ç‡ÇµÇÍÇ»Ç¢ÅB
		// (2005.10.8 yutaka)
		// ÉâÉCÉuÉâÉäÇÉäÉìÉNÇµÅAê≥ãKÇÃéËèáÇ≈ÉoÅ[ÉWÉáÉìÇìæÇÈÇ±Ç∆Ç…ÇµÇΩÅB
		// (2006.7.24 yutaka)
		...
		// static text ÇÃÉTÉCÉYÇïœçX (2007.4.16 maya)
		...
		w = LOWORD(dwExt) + 5; // ïùÇ™é·ä±ë´ÇËÇ»Ç¢ÇÃÇ≈ï‚ê≥
		h = HIWORD(dwExt);
		...
		// static textÇÉTÉuÉNÉâÉXâªÇ∑ÇÈÅBÇΩÇæÇµÅAtabstop, notifyÉvÉçÉpÉeÉBÇóLå¯Ç…ÇµÇƒÇ®Ç©Ç»Ç¢Ç∆
		// ÉÅÉbÉZÅ[ÉWÇ™èEÇ¶Ç»Ç¢ÅB(2005.4.5 yutaka)
...
BOOL CALLBACK GenDlg(HWND Dialog, UINT Message, WPARAM wParam, LPARAM lParam)
{
  ...
  switch (Message) {
	...
	case IDOK:
	  ...
	  // TTXKanjiMenu ÇÃÇΩÇﬂÇ…ÅAOK âüâ∫éûÇ…ÉÅÉjÉÖÅ[çƒï`âÊÇÃ
	  // ÉÅÉbÉZÅ[ÉWÇîÚÇŒÇ∑ÇÊÇ§Ç…ÇµÇΩÅB (2007.7.14 maya)
 ...
 * Revision 1.29  2007/08/13 22:18:47  maya
 * COM100 à»è„Ç™àµÇ¶Ç»Ç¢ñ‚ëËÇèCê≥ÇµÇΩÅB
 *
 * Revision 1.28  2007/08/08 15:57:56  maya
 * à¿ëSÇ»ä÷êîÇégópÇ∑ÇÈÇÊÇ§Ç…ïœçXÇµÇΩÅB
 *
 * Revision 1.27  2007/07/23 14:22:38  maya
 * ÉVÉäÉAÉãê⁄ë±ÇÃCOMç≈ëÂÉ|Å[ÉgÇ200Ç‹Ç≈ägí£ÇµÇΩÅB
 *
 * Revision 1.26  2007/07/20 21:55:27  maya
 * ÉVÉäÉAÉãÉ|Å[ÉgÉ_ÉCÉAÉçÉOÇ≈É{Å[ÉåÅ[ÉgÇ™ïœçXÇ≥ÇÍÇÈÇ±Ç∆Ç™Ç†ÇÈÇÃÇ≈ÅAèIóπéûÇ…É^ÉCÉgÉãÇïœçXÇ∑ÇÈÇÊÇ§Ç…ÇµÇΩÅB
 *
 * Revision 1.25  2007/07/14 00:22:52  maya
 * åæåÍÇ™ì˙ñ{åÍÇÃÇ∆Ç´ÇÃÇ› TTXKanjiMenu Ç™ï\é¶Ç≥ÇÍÇÈÇÊÇ§Ç…ÇµÇΩÅB
 *
 * Revision 1.24  2007/07/05 12:15:48  doda
 * óòópâ¬î\Ç»COMÉ|Å[ÉgÇ™ñ≥Ç¢éûÅAêVÇµÇ¢ê⁄ë±É_ÉCÉAÉçÉOÇ≈ÉVÉäÉAÉãÉ|Å[ÉgÇÃëIëÇñ≥å¯âªÇµÇΩÅB
 *
 * Revision 1.23  2007/07/02 10:50:43  doda
 * ãåÉoÅ[ÉWÉáÉìÇ∆ÇÃä‘Ç≈ÇÕã§óLÉÅÉÇÉäÇ…ÇÊÇÈê›íËÇÃã§óLÇ™èoóàÇ»Ç¢ÇΩÇﬂÅA
 * ÉtÉ@ÉCÉãÉ}ÉbÉsÉìÉOÉIÉuÉWÉFÉNÉgñºÇïœçXÇµÇΩÅB
 *
 ...
 * ïœçXå„ÇÃÉtÉHÉìÉgÉTÉCÉYÇéÊìæÇ∑ÇÈÇÊÇ§Ç…ïœçXÇµÇΩÅB
 *
 * Revision 1.20  2007/04/16 08:45:23  maya
 * URL Çä‹ÇﬁÉeÉLÉXÉgÇÃÉTÉCÉYÇìÆìIÇ…ïœçXÇ∑ÇÈÇÊÇ§Ç…ÇµÇΩÅB
 *
 ...
 * Terminal setupÉ_ÉCÉAÉçÉOÇÃ New-line ÇÃ Receive Ç… "LF" Çí«â¡ÇµÇΩÅB
 * éÛêMéûÇÃâ¸çsÉRÅ[ÉhÇ™ LF ÇÃèÍçáÇÕÅAÉTÅ[ÉoÇ©ÇÁ LF ÇÃÇ›Ç™ëóÇÁÇÍÇƒÇ≠ÇÈÇ∆
 * âºíËÇµÅACR+LFÇ∆ÇµÇƒàµÇ§ÇÊÇ§Ç…Ç∑ÇÈÅB
 *
 ...
 * Oniguruma 4.2.0Ç÷ç∑Çµë÷Ç¶ÇΩÇ±Ç∆Ç…çáÇÌÇπÇƒÅAÉoÅ[ÉWÉáÉìï\é¶ÇÃÇ≥ÇπÇ©ÇΩÇïœçXÇµÇΩÅB
 *
 * Revision 1.13  2006/03/12 14:27:41  yutakakn
 *   ÅEAdditional settingsÉ_ÉCÉAÉçÉOÇ…Ç®ÇØÇÈÉEÉBÉìÉhÉEÇÃîºìßñæïœçXÇë¶ç¿Ç…îΩâfÇ≥ÇπÇÈÇÊÇ§Ç…ÇµÇΩÅiteraterm.ini ÇÃ AlphaBlend=256 ÇÃèÍçáÇÃÇ›ÅjÅB
 *   ÅEï∂éöÇÃîwåiêFÇÉXÉNÉäÅ[ÉìÇÃîwåiêFÇ∆àÍívÇ≥ÇπÇÈÉpÉbÉ`ÇÃÉoÉOÇèCê≥ÇµÇΩÅBÉpÉbÉ`çÏê¨Ç…ä¥é”ÇµÇ‹Ç∑ÅÑ337éÅ
 *
 * Revision 1.12  2006/03/02 16:15:49  yutakakn
 *   ÅEï∂éöÇÃîwåiêFÇÉXÉNÉäÅ[ÉìÇÃîwåiêFÇ∆àÍívÇ≥ÇπÇÈÇÊÇ§Ç…ÇµÇΩÅBÇªÇÍÇ…Ç∆Ç‡Ç»Ç¢Window setupÉ_ÉCÉAÉçÉOÇ…"Always use Normal text's BG"É`ÉFÉbÉNÉ{ÉbÉNÉXÇí«â¡ÇµÇΩÅBÇ‹ÇΩÅAteraterm.iniÇ…UseNormalBGColorÉGÉìÉgÉäÇí«â¡ÇµÇΩÅBÉpÉbÉ`çÏê¨Ç…ä¥é”ÇµÇ‹Ç∑ÅÑ337éÅ
 *
 ...
 *   ÅEstrnicmp()Ç_strnicmp()Ç÷íuä∑ÇµÇΩ
 *   ÅEstrdup()Ç_strdup()Ç÷íuä∑ÇµÇΩ
 *   ÅEchdir()Ç_chdir()Ç÷íuä∑ÇµÇΩ
 *   ÅEgetcwd()Ç_getcwd()Ç÷íuä∑ÇµÇΩ
 *   ÅEstrupr()Ç_strupr()Ç÷íuä∑ÇµÇΩ
 *   ÅEtime_tÇÃ64ÉrÉbÉgâªÇ…Ç∆Ç‡Ç»Ç§åxçêÉÅÉbÉZÅ[ÉWÇó}é~ÇµÇΩ
 *   ÅETeraTerm MenuÇ™ÉrÉãÉhÉGÉâÅ[Ç∆Ç»ÇÈåªè€Ç…ëŒèàÇµÇΩ
 *   ÅEOniguruma 4.0.1Ç÷ç∑Çµë÷Ç¶ÇΩ
 *
 * Revision 1.10  2005/11/30 15:32:12  yutakakn
 * ÉVÉäÉAÉãê⁄ë±ÇÃCOMç≈ëÂÉ|Å[ÉgÇ99Ç‹Ç≈ägí£ÇµÇΩÅB
 * É{Å[ÉåÅ[ÉgÇ…230400, 460800, 921600Çí«â¡ÇµÇΩÅB
 *
 * Revision 1.9  2005/11/03 13:34:26  yutakakn
 *   ÅEteraterm.iniÇï€ë∂Ç∑ÇÈÇ∆Ç´Ç…èëÇ´çûÇ›Ç≈Ç´ÇÈÇ©Ç«Ç§Ç©ÇÃîªï Çí«â¡ÇµÇΩÅB
 *   ÅETCP/IP setupÉ_ÉCÉAÉçÉOÇÃ"Term type"ÇèÌÇ…óLå¯Ç∆Ç∑ÇÈÇÊÇ§Ç…ÇµÇΩÅB
 *
 * Revision 1.8  2005/10/08 14:56:06  yutakakn
 * OnigurumaÇÃÉoÅ[ÉWÉáÉìèÓïÒï\é¶Çè≠ÇµïœçXÅB
 *
 * Revision 1.7  2005/10/05 17:01:41  yutakakn
 * OnigurumaÇÃÉoÅ[ÉWÉáÉìÇÉoÅ[ÉWÉáÉìèÓïÒÇ…í«â¡ÅB
 *
 * Revision 1.6  2005/04/07 14:13:23  yutakakn
 * ÅEAdditional settingsÇ≈ÇÃÉ}ÉEÉXÉJÅ[É\ÉãéÌï Çê›íËéûÇ…ïœçXÇ≥ÇÍÇÈÇÊÇ§Ç…ÇµÇΩÅB
 * ÅEÉoÅ[ÉWÉáÉìèÓïÒÉ_ÉCÉAÉçÉOÇÃURLÇÉ_ÉuÉãÉNÉäÉbÉNÇ∑ÇÈÇ∆ÅAÉuÉâÉEÉUÇ™ãNìÆÇ≥ÇÍÇÈÇÊÇ§Ç…ÇµÇΩÅB
 *
 * Revision 1.5  2005/04/03 13:42:07  yutakakn
 * URLï∂éöóÒÇÉ_ÉuÉãÉNÉäÉbÉNÇ∑ÇÈÇ∆ÉuÉâÉEÉUÇ™ãNìÆÇ∑ÇÈÇµÇ©ÇØÇí«â¡ÅiêŒçËéÅÉpÉbÉ`Ç™ÉxÅ[ÉXÅjÅB
 *
 * Revision 1.4  2005/03/14 13:29:40  yutakakn
 * 2Ç¬Çﬂà»ç~ÇÃÉvÉçÉZÉXÇ≈ÅATeraTermÉoÅ[ÉWÉáÉìÇ™ê≥ÇµÇ≠éÊìæÇ≥ÇÍÇ»Ç¢ñ‚ëËÇ÷ÇÃëŒèàÅB
 *
 * Revision 1.3  2005/02/28 14:30:35  yutakakn
 * ÉoÅ[ÉWÉáÉìÉ_ÉCÉAÉçÉOÇ…ï\é¶Ç∑ÇÈTeraTermÇÃÉoÅ[ÉWÉáÉìÇÅAttermpro.exeÇÃ
 * ÉoÅ[ÉWÉáÉìèÓïÒÇ©ÇÁéÊìæÇ∑ÇÈÇÊÇ§Ç…ÇµÇΩÅB
 *
 * Revision 1.2  2004/11/28 13:14:51  yutakakn
 * ÉXÉNÉçÅ[ÉãÉoÉbÉtÉ@ÇÃì¸óÕåÖêîÇ5Ç©ÇÁ8Ç÷ägí£ÇµÇΩÅB
 * 
 */
